...
app.get('/createGroup', function(request, response) { //sends to create group page
 response.sendFile(path.join(__dirname + '/Create_Group.html'));
});
...
app.post('/createGroup', function(request, response) { // host puts info in, gets restaurant choices
  ...
  connection.query('INSERT INTO `Prototype1`.`Codes` (`CodeVal`,`userName`,`loginTime`,`category`,`distance`,`finished`) VALUES (?,?,NOW(),?,?,0);', [code,sess.username,category,distance], function(error, results, fields) {
  request.session.loggedin = true;
